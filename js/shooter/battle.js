...
var Battle = function() {
    ...
    this.units;
    this.map = {};
    this.log = new ui.list(0, 20, 80, 10, this);
    this.panel = new ui.panel(0, 18, 81, 2, this);
    this.info = new ui.panel(0, 30, 81, 8, this);
    this.lookPan = new ui.panel(2, 2, 60, 20, this);
    this.aimui = new Aim(this);
    this.hitboxpan = {};
    this.quit = false;
    this.closed = false;
    this.global = {};
    ...
    this.init = function() {
        ...
        this.hitboxpan = new ui.hitboxpanel(40, 3, this.player.body, this);
    ...
    this.control = function() {
        ...
        if (pAct == p.unitAct.aim) this.aim();
        if (pAct == p.unitAct.look) this.look();
    ...
    this.look = function() {
        ...
        this.aimui.reset();
        var res = this.aimui.get();
        if (!res || !this.map.fov(res.x, res.y)) return;
    ...
    this.aim = function() {
        ...
        var res = this.aimui.get();
        if (!this.map.fov(res.x, res.y)) return false;
        var u = this.map.getUnit(res.x, res.y);
        if (!u) return false;
